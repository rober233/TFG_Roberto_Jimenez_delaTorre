...
def ontology():
    ...
    #Cargar la ontologia de Protege
    onto = get_ontology("Archivos_OWL/ontologia_tfg.owl").load()
    # Cargar los datos del archivo CSV 
    df_activos = open("Archivos_CSV/activos.csv")
    reader_df_activos = csv.reader(df_activos)
    next(reader_df_activos)
    df_datos_TFG = open("Archivos_CSV/Datos_TFG_Basica.csv")
    reader_df_datos_TFG = csv.reader(df_datos_TFG)
    next(reader_df_datos_TFG)
    ...
    with onto:
        for row1 in reader_df_activos:
            ...
            onto.save("ontolgia_basica_tfg.owl")

    with onto:
            
            for row1 in reader_df_datos_TFG:
                INDEX,ataque,tipo,dlllist_avg_dlls_per_proc,handles_nfile,handles_nkey,handles_nthread,handles_nsemaphore,handles_ntimer,handles_nmutant,ldrmodules_not_in_load,ldrmodules_not_in_mem,ldrmodules_not_in_load_avg,ldrmodules_not_in_init_avg,ldrmodules_not_in_mem_avg,malfind_ninjections,malfind_commitCharge,malfind_protection,svcscan_nservices,svcscan_process_services,svcscan_nactive,callbacks_ncallbacks,impacto ,afecta_al_activo,genera_riesgo= row1
    ...
    with onto:
        ...
        r = list(default_world.sparql("""
                PREFIX tfg:<http://www.semanticweb.org/rober/ontolgia_tfg#>
        ...
        for x in range(len(r)):
                    for asset in onto.search(type=onto.Activo):
                        if asset == r[x][0]:
